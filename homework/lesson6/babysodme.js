//定义读取文件函数,需要把js文件放到words-from-the-heart中运行。
function scanFiles(){
    var fs = require('fs');
    var dirPathString = '../../';
...
function callbackScanFiles(err, files){
    ...
    if (files != undefined){
        var savefiles = files;
        callbackWriteFiles(savefiles);
    }else{
        console.log('读取的是空文件');
    }
};
//定一个写文件函数，用于存储读取文件的返回结果
function callbackWriteFiles(savefiles){
    if (savefiles != undefined){ 
    var fs = require('fs');
    var outputPathSting = './Write_file_async.txt';
    fs.writeFileSync(outputPathSting,savefiles);
    console.log('写文件成功')
